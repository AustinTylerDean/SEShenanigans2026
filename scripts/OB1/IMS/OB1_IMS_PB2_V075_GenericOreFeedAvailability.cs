...
bool FindBestOreForRefinery(bool isYield, bool yieldBank, bool highDemand, out string bestName, out double bestScore)
{
    ...
    {
        if (SoftBudgetHit()) { lastOptRefinery = "YIELD ORE FIND"; return false; }
        ...
        for (int j = 0; j < itemScratch.Count; j++)
        {
            ...
            if (Math.Floor((double)it.Amount) < 1.0) continue;
            string ore = OreName(it);
            if (!TrackedOre(ore)) continue;
...
double RefinedCurrent(string ore)
{
    ...
    if (ore.Equals("Stone", StringComparison.OrdinalIgnoreCase)) return GetPb1Double("Current.REFINED.GRAVEL", 0.0);
...
double RefinedMin(string ore)
{
    ...
    if (ore.Equals("Stone", StringComparison.OrdinalIgnoreCase)) return GetPb1Double("Target.REFINED.GRAVEL.Min", 0.0);
...
bool TrackedOre(string ore)
{
    for (int i = 0; i < ORE_NAMES.Length; i++)
        if (ore.Equals(ORE_NAMES[i], StringComparison.OrdinalIgnoreCase)) return true;
    return false;
}
...
void WriteStatus()
{
    ...
    b.AppendLine("WorkerVersion=OB1_PB2_V075_ORE_FEED_AVAILABILITY");
