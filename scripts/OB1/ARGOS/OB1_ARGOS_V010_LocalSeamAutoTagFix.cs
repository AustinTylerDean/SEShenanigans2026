// OB1 ARGOS V008 / SeamSafeMechanicalScope / EchoV003Style / From OB1 V004
...
//   - V010/V036 keeps seam-safe mechanical scope and ignores safe local dynamic seams such as tagged bay merge blocks connected to untagged prints.

const string ARG_TAG = "[ARG]";
const string DEFAULT_NAME = "UNNAMED";
const string DEFAULT_TAG = "";
const string SCRIPT_NAME = "ARGOS V010 OB1";
...
void LoadConfig()
{
    ...
    {
        _identityFault = true;
        _identityFaultText = "IDENTITY FAULT: name this PB with an entity tag, e.g. [OB1] ARGOS PB";
    }
...
bool ShouldIgnoreLocalDynamicSeam(MergePair candidate)
{
    ...
    bool aLocal = StartsWithEntityTag(candidate.A.CustomName);
    bool bLocal = StartsWithEntityTag(candidate.B.CustomName);
    if (aLocal == bLocal) return false;
    IMyShipMergeBlock other = aLocal ? candidate.B : candidate.A;
